...
## 并行上传之一取消并恢复（2026-09-17）

[真实实验](evidence/2026-09-17/parallel-cancel-resume.json) 在两个 9,900,000 字节文件使用不同会话重叠上传时，对第一个文件某分片的服务端成功应答触发 context 取消。独立客户端随后查到该分片的编号、非空 ETag、大小及原 uploadId；该文件正式路径仍404、日志Uploading、新写入被pending拒绝。另一个文件独立完成为Committed，且在恢复第一个文件之前已独立下载核对SHA-256。

显式Resume使用原K/uploadId，未重新初始化；两个文件最终完整下载与spool均匹配，最终两条Committed日志。整个实验仅两次multipart初始化和两次confirm，没有重复确认。这里的elapsed_seconds截止到初始两个请求结束，不包含后续显式恢复耗时，不能拿它表示全量恢复吞吐。context取消不等同于Finder取消、SIGKILL或整机掉电，系统验收状态不据此升级。

新增模拟HTTPS回归覆盖相同场景，并检查同路径持久拒绝、兄弟文件不中断、原会话恢复和每文件只初始化一次。完整Go race与vet通过。

## 真实 Finder 上传暴露零字节占位（2026-09-17）

用户恢复 Automation 权限后，实际检查确认 Accessibility 和真实 CGEvent 拖拽均可工作。历史 -1743 阻塞记录保留。容器访问另受 VPN 虚拟地址与其他 Docker 网段重叠影响，已通过[项目专用加密 DNS](docker-dns.md)恢复，不修改其他项目网络。

本次以真实 Finder 拖拽 `normal-upload-2.bin`（8,388,625 字节）至隔离云盘目录。录制严格限定两个已核验的测试 Finder 窗口，167.215 秒；实际动作附近画面已解码查看。切至其他 Space 时仅有黑色背景，未捕获其他应用。独立云端观察器在动作前确认正式路径 404，全程共 181 次观察，最终两次完整下载 SHA-256 均与本地输入相同。

**ST-001-T 为 FAIL**：11:36:47.049 至 11:36:58.748 UTC 的 90 次采样看到了零字节正式对象，随后才看到完整文件。后端日志也存在同一路径分别为 0 字节和完整大小的两条 Committed 记录。这不是上传成功的原子可见性证明，不能因为最终哈希正确而通过。见[脱敏失败摘要](evidence/2026-09-17/finder-normal-upload-failed.json)；完整录屏、逐次观察和过滤后的日志保存在本机 `evidence/ST-001-T/`。
...
## 文件 COPY 覆盖保护（2026-09-17）

上游 x/net/webdav 在 COPY 前删除已有目标，使后续上传被拒绝时旧文件已经丢失；另有源读失败后仍正常关闭目标、发布残缺前缀的问题。严格模式中已分别改为不预删文件和传递输入错误至 VFS CloseWithError，通用默认行为不变。新目录可递归复制；已有目录替换和文件/目录类型冲突拒绝，整树事务尚未完成，不能据此把 C-013 全部分支标 PASS。

[真实协议实验](evidence/2026-09-17/webdav-copy-overwrite.json) 使用全新隔离子根，5,200,000 字节源、15 字节旧目标：Overwrite:F 返回412；Overwrite:T 返回204，耗时约8.03秒。独立 SMH 客户端观察到18次完整旧版与2次完整新版，0次读取错误，未观察到404或其他内容；源完整下载哈希未变。新源、旧目标、覆盖目标三条上传日志均Committed，覆盖日志保留overwrite意图。有限采样不是服务端原子性的形式证明，也未测COPY提交丢响应、Finder复制或回收站/历史版本副作用；系统验收仍未通过。

完整上游 WebDAV race（32.087秒）及项目 race/vet 通过，补丁固定版本不变。测试服务已重建加载本轮修复，原测试挂载已重新核验恢复；没有进行鼠标操作。

## 路径保护加载到真实服务（2026-09-17）

服务随后重建至 `04692f0`，加载 COPY/MOVE 规范化路径和 URL 前缀占用修复。先卸载原生测试卷，再重启并等待健康检查，最后重新挂载核验。对既有隔离 fixture 执行目录 COPY/MOVE 到自身子目录，以及文件 COPY 到规范化后的自身，三个请求均403；5,200,000字节源文件前后完整读取SHA-256一致，794条JSON持久日志的名称和逐文件SHA-256完全未变。见[运行服务证据及二进制/补丁摘要](evidence/2026-09-17/webdav-path-boundaries-live.json)。

该实验只证明上述请求在运行服务中被拒绝且未改变已检查数据；带URL前缀的冲突行为由此前本地HTTP回归覆盖，不把当前根路径服务的结果泛化为全部配置。没有新增Finder录屏或系统PASS。
