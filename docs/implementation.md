...
- 注册 `sjtu` backend，提供元数据、分页、严格 Range/ETag 读取、受限创建目录、已知/未知长度的持久化上传；8 MiB 起使用分片协议。
...
下一步：完善分片自动恢复和会话过期行为；补充 1800 秒访问令牌自动刷新；继续调查可用的稳定身份、历史版本和 fs-journal 并发保护。真实证据应持续记录到 manifest，已知实现缺陷不能改写成符合预期。
