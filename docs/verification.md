...
这是可执行实现之前的验收规格，**不是测试运行报告**。所有用例未执行，P0 不得因当前实现失败而降低预期；SKIP/未知不计 PASS。当前没有后端和认证实验空间，暂不生成无真实驱动的占位 Karate/Go 测试。后端测试使用用户指定的 rclone/Go 体系；macOS 用户路径由原生 UI 和真实终端执行。
...
5. **并发条件**：读旧 content_cas/inode/version 后让另一客户端替换，再提交旧条件；验证条件针对源还是目标、错误码、失败是否零副作用。接口接受字段但忽略它应判失败。
...
| C-006 | 并发保存无静默丢更新 | 旧版本仍匹配 | ST-006-T：版本匹配 | ST-006-F：网页端或第二客户端先更新；另测 ABA 同路径重建 | 两个 Finder 客户端/编辑器覆盖同一路径 | setup / external-event / fact-check | 条件匹配才提交；冲突显式报告并保留两份数据 | 未执行；见 [006](api.md#4-新发现的高价值能力) |
...
| C-011 | 非空目录不会被误删 | 目录为空 | ST-011-T：空目录 | ST-011-F：非空目录；检查后并发新建子项 | 终端 rmdir 挂载目录 | setup / external-event / fact-check | 空目录删除；非空/竞态不递归删除用户内容 | 未执行；见 [011](api.md#4-新发现的高价值能力) |
...
| C-017 | 删除重试不误删新对象 | 路径仍指向原对象 | ST-017-T：删除成功且无重建 | ST-017-F：删成功响应丢失后路径已被重建 | Finder 删除后断网，网页端同路径创建新文件 | setup / external-event / fact-check | 不删除后来创建的文件；回收站项与操作身份一致 | 未执行；见 [017](api.md#5-幂等性与结果未知) |
