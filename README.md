...
**尚未达到 macOS 网盘发布标准。** 已执行部分真实云盘 API 实验；Finder、宿主机崩溃验收未完成。36 个系统验收分支没有任何一个被标记为通过。默认禁止覆盖，删除和目录删除仍未实现；隔离实验目录可显式启用创建，以及单客户端契约下的实验性顺序覆盖（lab_overwrite=true）。功能阻塞项和测试结果见 [实施进度](docs/implementation.md)。
